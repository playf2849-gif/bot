...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd == "add_number":
        ...
        save_data(data)
        # edit the start message to show the prompt
        ...
        # record awaiting code mapping by message id
        ...
        # user clicked "Проверил" — verify membership
        subscribed = await is_subscribed(user.id, context)
            if subscribed:
            # register user and show start
            data = load_data()
            username = f"@{user.username}" if user.username else user.first_name
            get_user_entry(data, user.id, username)
            save_data(data)
        ...
        # remove entry from queue and from user's numbers
        try:
        ...
        if not entry:
            ...
            return
        user_id = entry.get("user_id")
        # remove from queue and from user's numbers
        try:
            data["queue"] = [x for x in data.get("queue", []) if str(x.get("id")) != str(entry.get("id"))]
        except Exception:
            pass
...
def valid_number(text: str) -> bool:
    # accept + and digits, length 7..15 digits
    if not text:
        return False
    text = text.strip()
